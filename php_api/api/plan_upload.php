<?php
/**
 * Endpoint específico para carga de documentos en planes nutricionales
...
try {
    error_log("PLAN_UPLOAD: Iniciando carga de archivo. FILES=" . json_encode($_FILES));
    ...
    error_log("PLAN_UPLOAD: Archivo recibido: $plan_documento_nombre, tamaño=$tamaño bytes");
    ...
    if ($tipo_operacion === 'create') {
        ...
        if (is_null($codigo)) {
            ...
            $query = "INSERT INTO nu_plan_nutricional 
                      (codigo_paciente, desde, hasta, semanas, completado, codigo_entrevista, 
                       plan_documento, plan_documento_nombre, plan_indicaciones, 
                       plan_indicaciones_visible_usuario, url, fechaa, codusuarioa) 
                      VALUES 
                      (:codigo_paciente, :desde, :hasta, :semanas, :completado, :codigo_entrevista, 
                       :plan_documento, :plan_documento_nombre, :plan_indicaciones, 
                       :plan_indicaciones_visible_usuario, :url, NOW(), :codusuarioa)";
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(array("message" => "Plan creado con documento"));
            } else {
                http_response_code(503);
                echo json_encode(array("error" => "No se pudo insertar el plan"));
            }
        } else {
            http_response_code(400);
        ...
        $query = "UPDATE nu_plan_nutricional 
        ...
        if ($stmt->execute()) {
            ...
            http_response_code(503);
            echo json_encode(array("error" => "No se pudo actualizar el documento"));
        }
    }
