...
switch($request_method) {
    case 'GET':
        ...
        } else if(isset($_GET["consejo"])) {
            get_documentos_by_consejo($_GET["consejo"]);
        }
        break;
    case 'POST':
        create_documento();
...
function get_documentos_by_consejo($consejo_codigo) {
    global $db;
    
    $query = "SELECT codigo, codigo_consejo, tipo, nombre, url, orden
              FROM nu_consejo_documento
...
function get_documento($codigo) {
    ...
    $query = "SELECT codigo, codigo_consejo, tipo, nombre, 
              CASE 
    ...
    if ($documento) {
        // Asegurarse de que el documento esté en formato texto
...
function create_documento() {
    ...
    if(empty($data->codigo_consejo) || empty($data->tipo)) {
    ...
    $query = "INSERT INTO nu_consejo_documento SET
...
function update_documento() {
    ...
    $query = "UPDATE nu_consejo_documento SET
                tipo = :tipo,
                nombre = :nombre,
                documento = :documento,
                url = :url,
...
function delete_documento($codigo) {
    ...
    $query = "DELETE FROM nu_consejo_documento WHERE codigo = :codigo";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':codigo', $codigo);
