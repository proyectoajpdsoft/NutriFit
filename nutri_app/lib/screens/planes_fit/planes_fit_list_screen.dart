...
class _PlanesFitListScreenState extends State<PlanesFitListScreen> {
  ...
  bool _isTipoFit(dynamic raw) {
    final value = raw?.toString().trim().toLowerCase() ?? '';
    return value == 'fit' || value == 'plan_fit';
  }
  ...
  Future<void> _showCumplimientoDetalleFit(int codigoPaciente) async {
    try {
      ...
      final items = records
          .where((item) => _isTipoFit(item['tipo']))
          .map((item) {
        ...
        builder: (dialogContext) {
          ...
          return StatefulBuilder(
            builder: (context, setDialogState) {
              ...
              return AlertDialog(
                title: const Text('Cumplimiento Plan Fit'),
                content: SizedBox(
                  width: 460,
                  child: items.isEmpty
                      ? const Text(
