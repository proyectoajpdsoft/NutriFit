...
class EntrenamientoRepetitionsProgressChart extends StatefulWidget {
  const EntrenamientoRepetitionsProgressChart({
    super.key,
  ...
  @override
  State<EntrenamientoRepetitionsProgressChart> createState() =>
      _EntrenamientoRepetitionsProgressChartState();
}

class _EntrenamientoRepetitionsProgressChartState
    extends State<EntrenamientoRepetitionsProgressChart> {
  static const String _selectedSeriesPreferenceKey =
      'training_repetitions_progress_selected_series';
  static const String _selectionSearchVisiblePreferenceKey =
      'training_repetitions_progress_selection_search_visible';
  static const String _selectionSearchQueryPreferenceKey =
      'training_repetitions_progress_selection_search_query';
  ...
  List<_RepetitionActivityEntry> _activities = <_RepetitionActivityEntry>[];
  Map<String, _ExerciseRepetitionSeries> _seriesByKey =
      <String, _ExerciseRepetitionSeries>{};
  ...
  void didUpdateWidget(
    covariant EntrenamientoRepetitionsProgressChart oldWidget,
  ) {
  ...
  Future<void> _loadChartData() async {
    ...
    final activities = <_RepetitionActivityEntry>[];
    final seriesByKey = <String, _ExerciseRepetitionSeries>{};
    ...
    for (final entrenamiento in sortedEntrenamientos) {
      ...
      final realizadosConRepeticiones = ejercicios.where((ejercicio) {
        final repeticiones =
            (ejercicio.repeticionesRealizadas ?? ejercicio.repeticionesPlan) ??
                0;
        return ejercicio.realizado == 'S' && repeticiones > 0;
      }).toList();
      if (realizadosConRepeticiones.isEmpty) {
        continue;
      }

      final activityIndex = activities.length;
      activities.add(
        _RepetitionActivityEntry(
          index: activityIndex,
      ...
      for (final ejercicio in realizadosConRepeticiones) {
        final key = _exerciseKey(ejercicio);
        final nombre = _exerciseDisplayName(ejercicio);
        final serie = seriesByKey.putIfAbsent(
          key,
          () => _ExerciseRepetitionSeries(key: key, name: nombre),
        );
        final repeticiones = (ejercicio.repeticionesRealizadas ??
                ejercicio.repeticionesPlan ??
                0)
            .toDouble();
        serie.points.add(
          _ExerciseRepetitionPoint(
    ...
    final orderedKeys = seriesByKey.values.toList()
      ..sort((a, b) {
        ...
        final byRange = b.valueRange.compareTo(a.valueRange);
        if (byRange != 0) {
          return byRange;
        }
    ...
    setState(() {
      ...
      _orderedSeriesKeys = orderedKeys.map((serie) => serie.key).toList();
      _isLoading = false;
    });
  }
  ...
  String _selectionSubtitle(_ExerciseRepetitionSeries series) {
    final firstValue = series.points.first.repeticiones;
    final lastValue = series.points.last.repeticiones;
    return '${series.points.length} actividades · ${_formatValue(firstValue)} rep → ${_formatValue(lastValue)} rep';
  }
              ...
              content: SizedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Selecciona los ejercicios con repeticiones realizados más de una vez dentro del período actual. Seleccionados: ${selectedKeys.length} de ${_orderedSeriesKeys.length}${searchQuery.trim().isEmpty ? '' : ' · ${filteredKeys.length} coinciden con la búsqueda'}.',
                    ),
                    const SizedBox(height: 12),
                    Flexible(
  ...
  Future<void> _handleChartMenuSelection(
    String value,
    List<_ExerciseRepetitionTrendSummary> summaries,
  ) async {
    if (value == 'share') {
  ...
  String _formatBottomLabel(_RepetitionActivityEntry activity) {
  ...
  String _formatDelta(double value) {
    ...
    return '$prefix${_formatValue(value)} rep';
  }
  ...
  List<_ExerciseRepetitionTrendSummary> _buildTrendSummaries(
    Iterable<String> keys,
  ) {
    final groupedPoints = <String, List<_ExerciseRepetitionPoint>>{};
    ...
    for (final key in keys) {
      ...
      groupedPoints.putIfAbsent(groupKey, () => <_ExerciseRepetitionPoint>[])
        ..addAll(serie.points);
    }

    final summaries = groupedPoints.entries.map((entry) {
      final points = List<_ExerciseRepetitionPoint>.from(entry.value)
        ..sort((a, b) {
      ...
      final total =
          points.fold<double>(0, (sum, point) => sum + point.repeticiones);
      final firstValue = points.first.repeticiones;
      final lastValue = points.last.repeticiones;
      final averageValue = total / points.length;
      final deltaValue = lastValue - firstValue;
      final double percentChange =
          firstValue <= 0 ? 0.0 : (deltaValue / firstValue) * 100.0;

      return _ExerciseRepetitionTrendSummary(
    ...
    summaries.sort((a, b) {
      final byTrend = b.deltaValue.abs().compareTo(a.deltaValue.abs());
      if (byTrend != 0) {
        return byTrend;
      }
  ...
  Color _trendColor(_RepetitionTrend trend) {
    switch (trend) {
      case _RepetitionTrend.up:
        return Colors.green.shade700;
      case _RepetitionTrend.down:
        return Colors.red.shade700;
      case _RepetitionTrend.stable:
        return Colors.amber.shade800;
    }
  }

  String _trendLabel(_RepetitionTrend trend) {
    switch (trend) {
      case _RepetitionTrend.up:
        return 'Subiendo';
      case _RepetitionTrend.down:
  ...
  String _trendEmoji(_RepetitionTrend trend) {
    switch (trend) {
      case _RepetitionTrend.up:
        return '📈';
      case _RepetitionTrend.down:
  ...
  String _trendPdfIndicator(_RepetitionTrend trend) {
    switch (trend) {
      case _RepetitionTrend.up:
        return '↑ Subiendo';
      case _RepetitionTrend.down:
  ...
  Future<void> _exportAnalysisToExcel(
    ...
    final rows = summaries
        .map(
          (summary) => <String>[
            ...
            '${_formatValue(summary.averageValue)} rep',
            '${_formatValue(summary.firstValue)} rep',
            '${_formatValue(summary.lastValue)} rep',
            _formatDelta(summary.deltaValue),
    ...
    await TrainingWeightProgressExcelService.exportAnalysis(
      context: context,
      title: 'Analisis de evolucion de repeticiones',
  ...
  Future<void> _shareChartImage(
    ...
    try {
      ...
      final fileName =
          'grafica_repeticiones_${DateTime.now().millisecondsSinceEpoch}.png';
  ...
  Future<void> _generateProgressPdf(
    ...
    try {
      ...
      await TrainingProgressPdfService.generateProgressPdf(
        ...
        title: 'Evolución de repeticiones',
        periodLabel: _buildAnalysisPeriodLabel(),
        ...
        rows: summaries
            .map(
              (summary) => <String>[
                ...
                '${_formatValue(summary.firstValue)} rep',
                '${_formatValue(summary.lastValue)} rep',
                '${_formatDelta(summary.deltaValue)} (${summary.percentChange >= 0 ? '+' : ''}${_formatNumber(summary.percentChange, decimals: 1)}%)',
              ],
            )
            .toList(),
  ...
  Widget _buildActivityDetailCard(
    _ExerciseRepetitionPoint point,
    Color color,
  ) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              '${_formatValue(point.repeticiones)} rep',
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                    color: color,
  ...
  Future<void> _showExerciseDetailDialog(
    _ExerciseRepetitionTrendSummary summary,
  ) async {
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _buildMetricTag(
                      ...
                      label: 'Media',
                      value: '${_formatValue(summary.averageValue)} rep',
                      color: const Color(0xFF00897B),
                    ),
                    _buildMetricTag(
                      icon: Icons.play_arrow_rounded,
                      label: 'Inicio',
                      value: '${_formatValue(summary.firstValue)} rep',
                      color: const Color(0xFF5E35B1),
                    ),
                    _buildMetricTag(
                      icon: Icons.flag_rounded,
                      label: 'Final',
                      value: '${_formatValue(summary.lastValue)} rep',
  ...
  LineChartData _buildChartData(
    List<_ExerciseRepetitionSeries> visibleSeries,
    List<String> visibleKeys,
  ) {
    final maxY = visibleSeries
        .expand((serie) => serie.points)
        .map((point) => point.repeticiones)
      ...
      lineBarsData: visibleKeys.map((key) {
        ...
        return LineChartBarData(
          spots: series.points
              .map(
                (point) =>
                    FlSpot(point.activityIndex.toDouble(), point.repeticiones),
              )
              .toList(),
          isCurved: true,
          color: color,
  ...
  Widget _buildTrendChip(_ExerciseRepetitionTrendSummary summary) {
    ...
    return Container(
      ...
      child: Text(
        switch (summary.trend) {
          _RepetitionTrend.up => '⬆',
          _RepetitionTrend.down => '⬇',
          _RepetitionTrend.stable => '=',
        },
        style: TextStyle(
          color: color,
          fontWeight: FontWeight.w800,
  ...
  Widget _buildAnalysisCard(List<_ExerciseRepetitionTrendSummary> summaries) {
    return Card(
      elevation: 2,
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Tabla de evolución de repeticiones',
                          style: Theme.of(context)
                              .textTheme
                              .titleMedium
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'Tendencia, repeticiones medias, iniciales y finales.',
                          style:
  ...
  Widget build(BuildContext context) {
    ...
    if (_orderedSeriesKeys.isEmpty || _activities.isEmpty) {
      return _buildEmptyState(
        title: 'Sin repeticiones para representar',
        subtitle:
            'En este periodo no hay ejercicios realizados con repeticiones mayores que 0.',
      );
    }
    ...
    final visibleSeries = visibleKeys
        ...
        .whereType<_ExerciseRepetitionSeries>()
        .toList();
...
class _RepetitionActivityEntry {
  const _RepetitionActivityEntry({
    required this.index,
...
enum _RepetitionTrend { up, down, stable }

class _ExerciseRepetitionTrendSummary {
  const _ExerciseRepetitionTrendSummary({
  ...
  final double percentChange;
  final List<_ExerciseRepetitionPoint> points;

  _RepetitionTrend get trend {
    if (deltaValue > 0.5) {
      return _RepetitionTrend.up;
    }
    if (deltaValue < -0.5) {
      return _RepetitionTrend.down;
    }
    return _RepetitionTrend.stable;
  }
}

class _ExerciseRepetitionSeries {
  _ExerciseRepetitionSeries({required this.key, required this.name});

  final String key;
  final String name;
  final List<_ExerciseRepetitionPoint> points = <_ExerciseRepetitionPoint>[];

  int get changeScore {
    if (points.length < 2) return 0;
    var changes = 0;
    for (var index = 1; index < points.length; index++) {
      if (points[index].repeticiones != points[index - 1].repeticiones) {
        changes += 1;
      }
    }
    return changes;
  ...
  double get valueRange {
    ...
    final values = points.map((point) => point.repeticiones);
...
class _ExerciseRepetitionPoint {
  const _ExerciseRepetitionPoint({
